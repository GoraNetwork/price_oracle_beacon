...
from utils.gora_pyteal_utils import get_method_signature, SmartAssert
...
def approval_program(
    ...
    check_state_constraints = Seq([
        Assert(App.globalGet(global_keys["refund_request_made_percentage"]) <= Int(100)),
        Assert(App.globalGet(global_keys["refund_processing_percentage"]) <= Int(100)),
        Assert(App.globalGet(global_keys["algo_request_fee"]) <= Int(int(1_000_000_000_000))),
        Assert(App.globalGet(global_keys["gora_request_fee"]) <= Int(int(1_000_000_000_000_000))),
        Assert(App.globalGet(global_keys["voting_threshold"]) <= Int(100)),
        Assert(App.globalGet(global_keys["vote_refill_threshold"]) <= Int(int(1_000_000))),
        Assert(App.globalGet(global_keys["vote_refill_threshold"]) > Int(16)),
        Assert(App.globalGet(global_keys["vote_refill_amount"]) <= Int(int(1_000_000_000))),
    ])
    ...
    handle_creation = Seq([
        ...
        App.globalPut(global_keys["gora_request_fee"], Int(int(1_000_000_000))), # TODO: need response from Abdul and Ahmed
        App.globalPut(global_keys["voting_threshold"], Int(66)),
        App.globalPut(global_keys["time_lock"], Int(10)),
        App.globalPut(global_keys["vote_refill_threshold"], Int(20)),
        App.globalPut(global_keys["vote_refill_amount"], Int(int(10_000))),
    ...
    @Subroutine(TealType.none)
    def init(manager):
        is_creator = SmartAssert(Txn.sender() == Global.creator_address())
        ...
        return Seq([
            ...
            .Else(
                Seq([
                    ...
                    SmartAssert(Len(manager) == Int(32)),
                    SmartAssert(manager != Global.zero_address()),
    ...
    def request(request, destination, type, key, app_refs, asset_refs, account_refs, box_refs):
        ...
        return Seq([
            ...
            SmartAssert(App.box_create(new_key_hash.get(), Int(abi.size_of(RequestInfo)))),
            (request_box_cost := abi.Uint64()).set(MinBalance(Global.current_application_address()) - contract_min_balance.get()),
            SmartAssert(request_box_cost.get() > Int(0)),
            SmartAssert(account_algo >= App.globalGet(global_keys["algo_request_fee"]) + request_box_cost.get(), "NOT ENOUGH ALGO"),
            SmartAssert(account_token_amount >= App.globalGet(global_keys["gora_request_fee"])),
            App.localPut(Txn.sender(), local_keys["account_algo"], account_algo - (App.globalGet(global_keys["algo_request_fee"]) + request_box_cost.get())),
    ...
    def refund_request_box(requester):
        return Seq([
            ...
            (contract_min_balance := abi.Uint64()).set(MinBalance(Global.current_application_address())),
            SmartAssert(App.box_create(key_hash.get(),Int(abi.size_of(RequestInfo)))),
            (request_box_cost := abi.Uint64()).set(MinBalance(Global.current_application_address()) - contract_min_balance.get()),
            App.localPut(
                requester,
                local_keys["account_algo"],
                App.localGet(requester, local_keys["account_algo"]) + request_box_cost.get()
            ),
            SmartAssert(App.box_delete(key_hash.get())),
        ])
    ...
    def update_request_status(voting_app, request_key_hash, status, requester, proposal_bytes):
        ...
        update_request_box = Seq([
            SmartAssert(App.box_delete(key_hash.get())),
            SmartAssert(App.box_create(key_hash.get(),Int(abi.size_of(ProposalsEntry)))),
        ...
        return Seq([
            ...
            If(status_passed_in.get() == request_status["processing"]).
                Then(
                    ...
                    request_info_app,
                    Assert(request_info_app.hasValue() == Int(1)),
                ),
    ...
    def register_key(new_public_key):
        ...
        return If(
            ...
            Seq([
                Assert(
                    Len(new_public_key) == Int(32),
                    new_public_key != Global.zero_address()
                ),
                store_public_key,
                store_new_timestamp
            ])
    ...
    def update_rewards(rewards_account:Expr,previous_vote_bytes:Expr,previous_vote_requester:Expr):
        ...
        validate_previous_vote = Seq([
            ...
            .Then(Seq([
                ...
                Assert(get_completed_request_info.hasValue() == Int(1)),
                ...
                SmartAssert(completed_request_vote_count.get() > Int(0)),
                ...
                SmartAssert(completed_request_requester.get() != Global.zero_address()),
                ...
                .Then(
                    pending_rewards_points.store(
                        WideRatio(
                            [vote_count.get(),Int(1_000_000)],
                            [completed_request_vote_count.get()]
                        )
                    ),
                    ...
                    .Then(
                        refund_request_box(completed_request_requester.get())
                    )
                    .Else(Seq([
                        completed_request_info.set(
                            completed_request_hash,
                            completed_request_vote_count,
                            completed_request_stake_count,
                            completed_request_vote_round,
                            completed_request_rewards_payed,
                            completed_request_requester,
                            is_previous_history
                        ),
        ...
        return Seq([
            ...
            token_rewards_to_pay.store(WideRatio(
                [pending_rewards_points.load(),(App.globalGet(global_keys["gora_request_fee"]))],
                [Int(1_000_000)]
            )),
            App.localPut(
            ...
            #credit algo rewards
            algo_rewards_to_pay.store(WideRatio(
                [pending_rewards_points.load(),(App.globalGet(global_keys["algo_request_fee"]))],
                [Int(1_000_000)]
            )),
            App.localPut(
                rewards_account, local_keys["account_algo"], 
    ...
    def update_request_vote_tally():
        ...
        return Seq([
            Assert(vote_txn.type_enum() == TxnType.ApplicationCall),
            populate_request_info_tmps(request_key_hash),
    ...
    def claim_rewards_from_vote():
        ...
        amount_of_refill_threshold_txns = App.globalGet(global_keys["vote_refill_threshold"]) * Global.min_txn_fee()
        get_remaining_executions = contract_balance / amount_of_refill_threshold_txns
        ...
        return Seq([
            ...
            SmartAssert(amount_of_refill_threshold_txns > Int(0)),
    ...
    def deploy_voting_contract():
        algo_xfer = Gtxn[Txn.group_index() - Int(1)]
        ...
        return Seq([
            ...
            InnerTxnBuilder.Submit(),
            (cost_of_new_vote_contract := abi.Uint64()).set(
                MinBalance(Global.current_application_address()) - contract_min_bal.get()
            ),
            Assert(
                (algo_xfer.fee() + Txn.fee())>=Global.min_txn_fee()*Int(4),
                algo_xfer.type_enum()==TxnType.Payment,
                algo_xfer.receiver()==Global.current_application_address(),
                algo_xfer.amount()>=cost_of_new_vote_contract.get(),
                algo_xfer.close_remainder_to()==Global.zero_address(),
                algo_xfer.rekey_to()==Global.zero_address(),
                algo_xfer.lease()==Global.zero_address()
            ),
    ...
    deploy_voting_contract_selector = MethodSignature("deploy_voting_contract(pay)void")
