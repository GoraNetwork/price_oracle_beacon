...
export async function getGlobal( app_id: number, algodClient? : Algodv2) {
  ...
  output["pending_deposits"] = output_step["pd"].bytes !== "" ? decodeUint64(Buffer.from(output_step["pd"].bytes, "base64"), "safe") : 0;
...
export async function getLocal( app_id: number, address: string, algodClient? : Algodv2) {
  ...
  const local_non_stake = 
    {
      algo_rewards: Number(local_non_stake_tuple[0]),
      gora_rewards: Number(local_non_stake_tuple[1]),
      algo_non_stake: Number(local_non_stake_tuple[2]),
      gora_non_stake: Number(local_non_stake_tuple[3])
    };
...
export async function getPredictedLocal( app_id: number, main_app_id :number, address: string, algodClient? : Algodv2) {
  const output: any = {};
  algodClient = (algodClient === undefined ? bkr.clients.sandboxAlgod() : algodClient);
  ...
  let adjustment = 0;
  let local_state_main;
  try {
    local_state_main = await getMockMainLocal(main_app_id, getApplicationAddress(app_id), algodClient); 
  } catch (error) {
    local_state_main = await getLocalStateMain(getApplicationAddress(app_id), main_app_id, algodClient); 
  }
  ...
  if(last_aggregation["is_deposit"])
  {
    ...
    predicted_non_stake.gora_non_stake -= last_aggregation.amount;
...
export async function deploy_delegator_with_mock_main(testAsset: number)
{
  ...
  const suggestedParams = await sandboxAppClient.getSuggestedParams();
  ...
  await initApp.execute(sandboxAppClient.client,5);
...
export async function deploy_delegator(
  testAsset: number,
...
{   
  ...
  await sendGenericPayment(sandboxAccount.signer, sandboxAccount.addr, mainAppAddress, 1e6);
  await sendGenericPayment(sandboxAccount.signer, sandboxAccount.addr, appAddress, 1e6);
  const suggestedParams = await sandboxAppClient.getSuggestedParams();
  suggestedParams.flatFee = true;
