...
#LocalState------------------------------------------------------------------------
    ...
    lifetime_rewards: Final[AccountStateValue] = AccountStateValue(
        stack_type=TealType.uint64,
        default=Int(0),
        descr="lifetime rewards accumulated",
        key=LKEYMAP['lifetime_rewards'],
        static=False,
    )
    ...
    def init_app(
        ...
        return Seq([
            ...
            self.send_asset(asset.asset_id(), Global.current_application_address(), Int(0), Int(0)),
            ...
            Assert(aggregation_round_start.get() == Int(0)),
            ...
            (contract_min_bal_cost := abi.Uint64()).set(
                MinBalance(Global.current_application_address()) - contract_min_bal.get()
            ),
            Assert(
                algo_xfer.get().amount() == contract_min_bal_cost.get(),
            ...
            Assert(
                manager_address.get() != Global.zero_address(),
                Len(manager_address.get()) == Int(32)
            ),
            self.manager.set(manager_address.get()),
            Assert(
                manager_algo_share.get() <= Int(1000),
                manager_gora_share.get() <= Int(1000),
            ),
    ...
    def opt_in(
        self,
        vesting_app_id: abi.Uint64
    ):
        return Seq([
            self.initialize_account_state(),
            (round_info := TimeoutTracker()).decode(self.global_aggregation_round.get()),
            (current_round := abi.Uint64()).set(round_info.aggregation_round),
            self.last_update_time[Txn.sender()].set(current_round.get()),
            If(vesting_app_id.get() != Int(0))
            .Then(Seq([
                (stake_amount_default := abi.Uint64()).set(Int(0)),
                (vesting_info := VestingTracker()).set(
                    stake_amount_default,
                    vesting_app_id
                ),
                self.vesting_tracker[Txn.sender()].set(vesting_info.encode())
            ]))
        ])
        ...
        return Seq([
            ...
            If(aggregation_round_round.get() != Int(0)).
                Then(Seq([
                    ...
                    self.update_local_aggregation_tracker(Int(0), Int(0)),
            ])),
    ...
    def stake(
        ...
        return Seq([
            Assert(asset_pay.get().asset_receiver() == Global.current_application_address()),
            If(And(
                vesting_on_behalf_of.address() != Global.zero_address(),
                vesting_on_behalf_of.address() != Txn.sender()
            )).
            Then(
                ...
                Assert(
                    vesting_app.get() == Global.caller_app_id(),
                    vesting_app.get() != Int(0)
                ),
                vesting_amount.set(vesting_amount.get() + asset_pay.get().asset_amount()),
                vesting_info.set(vesting_amount, vesting_app),
            ...
            self.update_local_aggregation_tracker(asset_pay.get().asset_amount(), Int(1)),
    ...
    def unstake(
        ...
        return Seq([         
            ...
            self.update_local_aggregation_tracker(amount_to_withdraw.get(), Int(0)),
            self.post_action_update(),
            Assert(
                amount_to_withdraw.get() <= Btoi(self.local_stake[self.userAddr.load()].get()),
                amount_to_withdraw.get() >= vesting_amount.get()
            )
        ])
    ...
    @internal(TealType.none)
    def update_local_aggregation_tracker(
        self,
        amount_arg,
        is_stake_arg
    ):
    ...
    def user_claim(
        ...
        return Seq([
            self.userAddr.store(Txn.sender()),
            opup.ensure_budget(Int(1000),OpUpFeeSource.GroupCredit),
            Assert(Txn.fee() >= Int(4) * Global.min_txn_fee()),
            self.post_action_update(),
            ...
            Assert(
                Or(
                    algo_to_withdraw.get() > Int(0),
                    gora_to_withdraw.get() > Int(0)
                )
            ),
            (algo_non_stake:= abi.Uint64()).set(current_funds.algo_non_stake),
            (gora_non_stake := abi.Uint64()).set(current_funds.gora_non_stake),

            self.send_algo(Txn.sender(), algo_to_withdraw.get(), Int(0)),
            self.send_asset(GORA_TOKEN_ID, Txn.sender(), gora_to_withdraw.get(),Int(0)),
            (zero := abi.Uint64()).set(Int(0)),
            # Since we have sent them all available rewards. The current funds will be reset to 0 for rewards
            current_funds.set(zero, zero, algo_non_stake, gora_non_stake),
            self.local_non_stake[self.userAddr.load()].set(current_funds.encode()),
            If(
                Int(18_446_744_073_709_551_615) - 
                self.lifetime_rewards[self.userAddr.load()].get() > 
                gora_to_withdraw.get()
                # Itob(self.lifetime_rewards[self.userAddr.load()].get() +
                # gora_to_withdraw.get()) < Itob(Int(18_446_744_073_709_551_615))
            )
            .Then(
                self.lifetime_rewards[self.userAddr.load()].set(
                    self.lifetime_rewards[self.userAddr.load()].get() + 
                    gora_to_withdraw.get()
                ),
            ),
    ...
    def goracle_claim(
        ...
        return Seq([
            ...
            If(App.optedIn(self.manager.get(), Global.current_application_id())).
                Then(
                    Seq([
                        (manager_historical_account_rewards := RewardsTracker()).decode(self.local_non_stake[self.manager.get()].get()),
                        (manager_historical_algo := abi.Uint64()).set(manager_historical_account_rewards.algo_rewards),
                        (manager_historical_gora := abi.Uint64()).set(manager_historical_account_rewards.gora_rewards),

                        (manager_historical_nonstake_algo := abi.Uint64()).set(manager_historical_account_rewards.algo_non_stake),
                        (manager_historical_nonstake_gora := abi.Uint64()).set(manager_historical_account_rewards.gora_non_stake),

                        manager_share_algo.set(WideRatio(
                            [get_unclaimed_rewards_algo.value(),self.manager_algo_share.get()],
                            [Int(1000)]
                        )),
                        manager_share_gora.set(WideRatio(
                            [get_unclaimed_rewards_gora.value(),self.manager_gora_share.get()],
                            [Int(1000)]
                        )),
                        (manager_algo := abi.Uint64()).set(manager_historical_algo.get() + manager_share_algo.get()),
                        (manager_gora := abi.Uint64()).set(manager_historical_gora.get() + manager_share_gora.get()),
                        (manager_account_rewards := RewardsTracker()).set(manager_algo, manager_gora, manager_historical_nonstake_algo, manager_historical_nonstake_gora),
            ...
            (historical_algo := abi.Uint64()).set(rewards_accumulation.algo_rewards),
            (historical_gora := abi.Uint64()).set(rewards_accumulation.gora_rewards),
            (historical_nonstake_algo := abi.Uint64()).set(rewards_accumulation.algo_non_stake),
            (historical_nonstake_gora := abi.Uint64()).set(rewards_accumulation.gora_non_stake),
            (algo := abi.Uint64()).set(get_unclaimed_rewards_algo.value() + historical_algo.get() - manager_share_algo.get()),
            (gora := abi.Uint64()).set(get_unclaimed_rewards_gora.value() + historical_gora.get() - manager_share_gora.get()),

            (account_rewards := RewardsTracker()).set(algo, gora, historical_nonstake_algo, historical_nonstake_gora),
    ...
    def calculate_rewards(
        ...
        return Seq([
            ...
            If(And(
                Btoi(self.global_stake_time.get()) != Int(0),
                Btoi(self.global_stake_time.get()) != adjustment.load(),
                Btoi(self.global_stake_time.get()) > adjustment.load()
            )).Then(Seq([
                share_algo.set(WideRatio(
                    [new_rewards_algo.get(),Btoi(tmp_stake_time.load())],
                    [Btoi(self.global_stake_time.get()) - adjustment.load()]
                )),
                share_gora.set(WideRatio(
                    [new_rewards_gora.get(),Btoi(tmp_stake_time.load())],
                    [Btoi(self.global_stake_time.get()) - adjustment.load()]
                )),
            ])),
    ...
    def register_participation_key(
        ...
        return Seq([
            Assert(
                Txn.sender() == self.manager.get(),
                new_key.get() != Global.zero_address(),
                Len(new_key.get()) == Int(32)
            ),
    ...
    def configure_settings(
        ...
        return Seq([
            ...
            Assert(manager_address.get() != Global.zero_address()),
            Assert(Len(manager_address.get()) == Int(32)),
            self.manager.set(manager_address.get()),
            Assert(
                manager_algo_share.get() <= Int(1000),
                manager_gora_share.get() <= Int(1000),
    ...
    def withdraw_non_stake(
        ...
        return Seq([
            opup.ensure_budget(Int(1000),fee_source=OpUpFeeSource.GroupCredit),            
            ...
            Else( #if staking from a normal wallet
                self.userAddr.store(Txn.sender()),
            ),
            ...
            (algo_to_withdraw := abi.Uint64()).set(account_non_stake.algo_non_stake),
            (gora_to_withdraw := abi.Uint64()).set(account_non_stake.gora_non_stake),
            ...
            Then(
                ...
                (old_gora_to_withdraw := abi.Uint64()).set(gora_to_withdraw),
                ...
                If(algo_to_withdraw.get() > Int(0)).Then(self.send_algo(Txn.sender(), algo_to_withdraw.get(), Int(0))),
                If(gora_to_withdraw.get() > Int(0)).Then(self.send_asset(GORA_TOKEN_ID , Txn.sender(), gora_to_withdraw.get(), Int(0))),

                (gora_remainder := abi.Uint64()).set(old_gora_to_withdraw.get() - gora_to_withdraw.get()),
                (zero := abi.Uint64()).set(Int(0)),
                account_non_stake.set(algo_rewards, gora_rewards, zero,gora_remainder),
            ...
            Then(
                ...
                Assert(vesting_amount.get() <= gora_to_withdraw.get()),
                self.send_asset(GORA_TOKEN_ID , Txn.sender(), vesting_amount.get(), Int(0)),
                (gora_remainder := abi.Uint64()).set(gora_to_withdraw.get() - vesting_amount.get()),
                account_non_stake.set(algo_rewards, gora_rewards, algo_to_withdraw, gora_remainder),
                self.local_non_stake[self.userAddr.load()].set(account_non_stake.encode()),
