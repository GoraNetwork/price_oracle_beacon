...
def approval_program(CONTRACT_VERSION, VOTE_VERIFY_LSIG_ADDRESS, DEV_MODE=False):
    ...
    def get_primary_account(primary_account_arg_index:Expr):
        ...
        return Seq([
            ...
            SmartAssert(local_stake_account_pk.hasValue() == Int(1)),
    ...
    def get_local_stake(request_round:abi.Uint64):
        ...
        return Seq([
            local_stake_array,
            SmartAssert(local_stake_array.hasValue() == Int(1)),
    ...
    def update_request_status(
        ...
        return Seq([
            # Fees are here as this is accounted for in the request fee and the refill mechanism
    ...
    @Subroutine(TealType.none)
    def validate_register_payment_txn(total_box_cost: abi.Uint64):
        ...
        return Seq([
            ...
            SmartAssert(payment_txn.amount()==total_box_cost.get()),
    ...
    def register_voter():
        return Seq([
            ...
            SmartAssert(App.box_create(get_primary_account(Int(1)),Int(abi.size_of(ProposalsEntry))) == Int(1)),
            (proposal_box_cost := abi.Uint64()).set(MinBalance(Global.current_application_address()) - contract_min_balance.get()),
            SmartAssert(proposal_box_cost.get() > Int(0)),
            # This create/delete here is to dynamically get the cost of a proposal when registering and avoid a duplicate key situation, etc
            SmartAssert(App.box_delete(get_primary_account(Int(1))) == Int(1)),

            contract_min_balance.set(MinBalance(Global.current_application_address())),
            SmartAssert(App.box_create(get_primary_account(Int(1)),Int(abi.size_of(LocalHistoryEntry))) == Int(1)),
            (total_box_cost := abi.Uint64()).set(
                MinBalance(Global.current_application_address()) - contract_min_balance.get() + proposal_box_cost.get()
            ),
            SmartAssert(total_box_cost.get() > Int(0)),
            validate_register_payment_txn(total_box_cost),
    ...
    def deregister_voter():
        return Seq([
            ...
            SmartAssert(box_cost.get() > Int(0)),
            contract_min_balance.set(MinBalance(Global.current_application_address())),
            box_cost.set(box_cost.get() + (contract_min_balance.get() - MinBalance(Global.current_application_address()))),
            InnerTxnBuilder.Begin(),
            InnerTxnBuilder.SetFields({
        ...
        return Seq([
            ...
            .Then(
                ...
                Seq([
                    ...
                    SmartAssert(box_cost.get() > Int(0)),
                    # Fees are here as this is accounted for in the request fee and the refill mechanism
                    If(box_cost.get() >= Global.min_txn_fee()) # TODO: probably want to be at least equal to, to allow deleting boxes no matter what.
                    .Then(
                        InnerTxnBuilder.Begin(),
                        InnerTxnBuilder.SetFields({
                            TxnField.type_enum: TxnType.Payment,
                            TxnField.amount: box_cost.get() - Global.min_txn_fee(),
                            TxnField.fee: Global.min_txn_fee(),
    ...
    def validate_vote_verify_txn(vrf_result, vrf_proof, vote_round):
        ...
        return Seq([
            ...
            And(
                sender_previous_vote.hasValue() == Int(1),
    ...
    def verify_vote_count(vote_count_arg, vrf_result, z_index):
        ...
        return Seq([
            SmartAssert(Int(table_size) > Int(0)),
            If(q < uint64_half)
    ...
    def vote(
        ...
        previous_vote_info = Seq([
            ...
            Assert(previous_vote.hasValue() == Int(1)),
        ...
        get_voting_threshold = Seq([
            voting_threshold,
            Assert(voting_threshold.hasValue() == Int(1)),
        ...
        start_new_round = Seq([
            ...
            App.globalPut(global_keys["current_request_vote_threshold"], WideRatio([get_voting_threshold, total_stake_abi.get()],[Int(1000)]))
        ])

        time_lock = App.globalGetEx(MAIN_APP, main_global_keys['time_lock'])
        get_time_lock = Seq([
            time_lock,
            Assert(time_lock.hasValue() == Int(1)),
            time_lock.value()
        ])
        ...
        is_valid_participant = Seq([
            ...
            SmartAssert( stored_timestamp.hasValue() == Int(1)),
        ...
        process_incoming_vote_and_update_values = Seq([
            ...
            (zero := abi.Uint64()).set(0),
            SmartAssert(Len(requester_account) == Int(32)),
            (requester_address := abi.make(abi.Address)).set(requester_account),
            SmartAssert(requester_address.get() != Global.zero_address()),
        ...
        return Seq([
            ...
            If(vote_count_abi.get() * Int(100) >= App.globalGet(global_keys["current_request_vote_threshold"]))
            .Then(
                process_request
            )
        ])
        ...
        [
            ...
            Seq([
                ...
                vote(
                    ...
                    Txn.applications[Btoi(Txn.application_args[4])], # destination_app_id
