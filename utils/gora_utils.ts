...
export async function generateInnerTxnID(txnID: string, client: Algodv2){
  ...
  const outerTxnResult = await client.pendingTransactionInformation(txnID).do()
  const encodedOuterTxn = outerTxnResult.txn.txn as EncodedTransaction
  const outerTxn = Transaction.from_obj_for_encoding(encodedOuterTxn)

  const innerTxnIds: Uint8Array[] = []

  for (let i = 0; i < outerTxnResult["inner-txns"].length; i++){
    const innerTxn = outerTxnResult["inner-txns"][i].txn.txn as EncodedTransaction

    let input = concatArrays(Buffer.from("TX"),outerTxn.rawTxID())
    const indexUint64 = encodeUint64(i)
    input = concatArrays(input, indexUint64)
    // Serialize the transaction using msgpack
    const serializedTxn = encodeObj(innerTxn);
    input = concatArrays(input,serializedTxn)
    // Hash the transaction using SHA-512/256
    const hash = crypto.createHash('sha512-256');
    const txnID = hash.update(input).digest();
    // Return the transaction ID as a Uint8Array
    innerTxnIds.push(new Uint8Array(txnID))
  }
...
export async function getVoteHash(
  ...
  userData:string | Uint8Array,
  errorCode:number,
  bitField:number,
){
  let userDataInput;
  if(typeof userData == "string"){
    userDataInput = Buffer.from(userData)
  } else {
